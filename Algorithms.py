...
class ETD_LCBT_NonIID_Window:
    ...
    def _compute_alpha_from_empirical_cdf(self,exp_sample):
        sum=0
        sample=50
        sample=10000
        ...
        for k in range(sample):
            ...
            # print(self.l_n,self.n)
            items = np.vstack([exp_sample, self.items[self.l_n:self.n]])
            sum+=(1/2)*max([x.dot(self.theta_hat) for x in items])
        self.alpha=(1/sample)*sum
        print('alpha',self.alpha)
    def run(self, t, x, y,l,h):
        self.lows=l
        ...
        if t ==self.l_n+1:
            self.theta_hat = np.linalg.solve(self.V, self.b)
            self.Vinv = np.linalg.inv(self.V)
            self._compute_alpha_from_empirical_cdf(self.sample)
